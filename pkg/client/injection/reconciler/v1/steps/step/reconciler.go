...
import (
	...
	v1steps "github.com/tektoncd/pipeline/pkg/client/listers/v1/steps"
	zap "go.uber.org/zap"
...
// controller reconciling steps.Step.
type Interface interface {
	// ReconcileKind implements custom logic to reconcile steps.Step. Any changes
	// to the objects .Status or .Finalizers will be propagated to the stored
	// object. It is recommended that implementors do not call any update calls
	// for the Kind inside of ReconcileKind, it is the responsibility of the calling
	// controller to propagate those properties. The resource passed to ReconcileKind
	// will always have an empty deletion timestamp.
	ReconcileKind(ctx context.Context, o *steps.Step) reconciler.Event
}

// Finalizer defines the strongly typed interfaces to be implemented by a
// controller finalizing steps.Step.
type Finalizer interface {
	// FinalizeKind implements custom logic to finalize steps.Step. Any changes
	...
	// deletion timestamp.
	FinalizeKind(ctx context.Context, o *steps.Step) reconciler.Event
}

// ReadOnlyInterface defines the strongly typed interfaces to be implemented by a
// controller reconciling steps.Step if they want to process resources for which
// they are not the leader.
type ReadOnlyInterface interface {
	// ObserveKind implements logic to observe steps.Step.
	// This method should not write to the API.
	ObserveKind(ctx context.Context, o *steps.Step) reconciler.Event
}

type doReconcile func(ctx context.Context, o *steps.Step) reconciler.Event

// reconcilerImpl implements controller.Reconciler for steps.Step resources.
...
func NewReconciler(ctx context.Context, logger *zap.SugaredLogger, client versioned.Interface, lister v1steps.StepLister, recorder record.EventRecorder, r Interface, options ...controller.Options) controller.Reconciler {
...
func (r *reconcilerImpl) Reconcile(ctx context.Context, key string) error {
	...
	getter := r.Lister.Steps(s.namespace)
...
func (r *reconcilerImpl) updateFinalizersFiltered(ctx context.Context, resource *steps.Step, desiredFinalizers sets.String) (*steps.Step, error) {
	// Don't modify the informers copy.
	...
	patcher := r.Client.TektonSteps().Steps(resource.Namespace)
...
func (r *reconcilerImpl) setFinalizerIfFinalizer(ctx context.Context, resource *steps.Step) (*steps.Step, error) {
...
func (r *reconcilerImpl) clearFinalizer(ctx context.Context, resource *steps.Step, reconcileEvent reconciler.Event) (*steps.Step, error) {
	if _, ok := r.reconciler.(Finalizer); !ok {
		return resource, nil
	}
	if resource.GetDeletionTimestamp().IsZero() {
