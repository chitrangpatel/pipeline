...
type StepLister interface {
	...
	List(selector labels.Selector) (ret []*steps.Step, err error)
...
// List lists all Steps in the indexer.
func (s *stepLister) List(selector labels.Selector) (ret []*steps.Step, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*steps.Step))
	})
	return ret, err
}
...
type StepNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*steps.Step, error)
...
// List lists all Steps in the indexer for a given namespace.
func (s stepNamespaceLister) List(selector labels.Selector) (ret []*steps.Step, err error) {
...
// Get retrieves the Step from the indexer for a given namespace and name.
func (s stepNamespaceLister) Get(name string) (*steps.Step, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(steps.Resource("step"), name)
	}
	return obj.(*steps.Step), nil
}
