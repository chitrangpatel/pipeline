...
func createResultsSidecar(taskSpec v1beta1.TaskSpec) v1beta1.Sidecar {
	resultsStr := ""
	for _, tres := range taskSpec.Results {
		resultsStr = fmt.Sprintf("%s%s ", resultsStr, tres.Name)
	}
	resultsSidecarScript := fmt.Sprintf(`#! /bin/bash
resultsPath=%s
checkedoff=()                  
expected=( %s)
...
do
   ...
   if [[ "$arraysAreSame" = true ]]; then
      break
   fi
   sleep 0.1
done`, "/tekton/results", resultsStr)

	return v1beta1.Sidecar{
		Name:   "results",
		Image:  "ubuntu",
		Script: resultsSidecarScript,
	}
}
...
func (b *Builder) Build(ctx context.Context, taskRun *v1beta1.TaskRun, taskSpec v1beta1.TaskSpec, transformers ...Transformer) (*corev1.Pod, error) {
	...
	if sidecarLogsResultsEnabled && taskSpec.Results != nil {
		...
		resultsSidecar := createResultsSidecar(taskSpec)
	...
	if alphaAPIEnabled {
		stepContainers, err = orderContainers(credEntrypointArgs, stepContainers, &taskSpec, taskRun.Spec.Debug, !readyImmediately, sidecarLogsResultsEnabled)
	} else {
		stepContainers, err = orderContainers(credEntrypointArgs, stepContainers, &taskSpec, nil, !readyImmediately, sidecarLogsResultsEnabled)
	}
	if err != nil {
		return nil, err
	}
	...
	if sidecarLogsResultsEnabled {
		...
		// so that they can access /tekton/results.
